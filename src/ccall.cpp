...
static Value *runtime_apply_type_env(jl_codectx_t &ctx, jl_value_t *ty)
{
    ...
    call->addRetAttr(Attribute::getWithAlignment(jl_LLVMContext, Align(16)));
    return call;
}
...
std::string generate_func_sig(const char *fname)
{
    ...
    for (size_t i = 0; i < nccallargs + sret; ++i) {
        ...
        if (!as.hasAttributes())
            continue;
        attributes = attributes.addAttributesAtIndex(jl_LLVMContext, i + 1, as);
    }
    // If return value is boxed it must be non-null.
    if (retboxed)
        attributes = attributes.addRetAttribute(jl_LLVMContext, Attribute::NonNull);
    if (rt == jl_bottom_type) {
        attributes = attributes.addFnAttribute(jl_LLVMContext, Attribute::NoReturn);
    }
    return "";
}
};
